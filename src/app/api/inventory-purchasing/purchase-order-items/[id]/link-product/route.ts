...
import { linkPurchaseOrderItemProduct } from '@/lib/purchaseOrders'
...
export async function PATCH(
  ...
  try {
    const order = await linkPurchaseOrderItemProduct(params.id, body.productId)
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : '关联商品失败' },
      { status: 400 },
    )
  }
}
