...
export default function ProductsPage() {
  ...
  const getNextAction = (p: Product) => {
    if (isMissing(p)) return { label: '补全信息', hint: '补图片/卖点/成本/售价' }
    if (isPriceAbnormal(p)) return { label: '检查价格', hint: '核对 TikTok 与本地定价' }
    if (isLowProfit(p)) return { label: '提升毛利', hint: '调折扣/物流/佣金/广告' }
    if (isLowStock(p)) return { label: '补库存', hint: `库存低于阈值` }
            ...
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredProducts.map((product) => {
                  ...
                  const costDetailTooltip = `头程: $${product.firstLegLogisticsCostUsd.toFixed(
                    2,
                  )} | 尾程: $${product.lastLegLogisticsCostUsd.toFixed(
                    2,
                  )} | 折扣价: ${
                    product.discountPriceUsd != null ? `$${product.discountPriceUsd.toFixed(2)}` : '-'
                  } | 达人佣金: $${product.influencerCommissionUsd.toFixed(
                    2,
                  )} | 广告费用: $${product.adCostUsd.toFixed(2)}`
                    ...
                    <td className="px-3 py-2 align-middle">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <div className="font-medium text-gray-900 truncate max-w-[220px]">{product.name}</div>
                          <div className="mt-0.5 flex items-center gap-2 text-xs text-gray-500">
                            <span className="truncate max-w-[160px]">{category}</span>
                            <button
                              type="button"
                              title={costDetailTooltip}
                              onClick={() =>
                                setExpandedId(prev => (prev === product.id ? null : product.id))
                              }
                              className="inline-flex items-center px-1.5 py-0.5 rounded-full border border-gray-200 hover:bg-gray-100"
                            >
                              <span className="text-[11px] text-gray-600">成本明细</span>
                            </button>
                          </div>
                        </div>
                      </div>
                    </td>
                    <td className="px-3 py-2 text-xs text-gray-500 align-middle">{product.sku || '-'}</td>
                    {/* 成本价，可编辑 */}
                    <td className="px-3 py-2 align-middle">
                      {canEdit ? (
                        <input
                    ...
                    <td className="px-3 py-2 align-middle">
                      <div className="flex flex-col gap-1">
                        <div className="inline-flex items-center gap-2">
                          <span
                            className="inline-flex items-center px-2 py-0.5 rounded-full border border-primary-100 bg-primary-50 text-xs text-primary-700"
                            title={action.hint}
                          >
                            {action.label}
                          </span>
                  ...
                  {expandedId === product.id && (
                    <tr className="bg-gray-50/60">
                      <td colSpan={9} className="px-3 py-2 text-xs text-gray-700">
                        <div className="flex flex-wrap gap-x-6 gap-y-2 items-center">
                          <div className="flex items-center gap-2">
                            <span className="text-gray-500">头程物流(USD)</span>
                            {canEdit ? (
                              <input
                                type="number"
                              ...
                              <span>${product.firstLegLogisticsCostUsd.toFixed(2)}</span>
                            )}
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-gray-500">尾程物流(USD)</span>
                            {canEdit ? (
                              <input
                                type="number"
                                className="w-20 px-2 py-0.5 border rounded text-xs"
                              ...
                              <span>${product.lastLegLogisticsCostUsd.toFixed(2)}</span>
                            )}
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-gray-500">折扣价(USD)</span>
                            {canEdit ? (
                              <input
                                type="number"
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-gray-500">达人佣金(USD)</span>
                            {canEdit ? (
                              <input
                                type="number"
                                className="w-20 px-2 py-0.5 border rounded text-xs"
                              ...
                              <span>${product.influencerCommissionUsd.toFixed(2)}</span>
                            )}
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-gray-500">广告费用(USD)</span>
                            {canEdit ? (
                              <input
                                type="number"
                                className="w-20 px-2 py-0.5 border rounded text-xs"
                              ...
                              <span>${product.adCostUsd.toFixed(2)}</span>
                            )}
                          </div>
                        </div>
                      </td>
